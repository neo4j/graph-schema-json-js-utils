...
describe("Validate if JSON-documen is a string", () => {
  ...
  test("Identifies if JSON schema is a string and is parseable to JSON", () => {
    ...
    assert.throws(
      () => validateSchema(NON_JSON, fullSchema), InputTypeError);
  });

  test("Identifies if graph schema is a string and is parseble JSON", () => {

    assert.throws(() => validateSchema(JSON_SCHEMA, NON_STRING), InputTypeError);
    assert.throws(
      () => validateSchema(JSON_SCHEMA, NON_JSON), InputTypeError);
